...
function getWellbeingInfo(riskLevel: string | undefined): WellbeingInfo {
  ...
  switch (riskLevel.toLowerCase()) {
    case "low":
      return { label: "Stable condition", sub: "No immediate concerns detected.", cls: "low" };
    case "medium":
      return { label: "Needs closer follow-up", sub: "Some concerns were detected.", cls: "medium" };
    case "high":
      return { label: "Attention recommended", sub: "Support may be needed soon.", cls: "high" };
    case "critical":
      return { label: "Urgent attention needed", sub: "Immediate action may be required.", cls: "critical" };
    default:
      return { label: "No recent assessment", sub: "Start or continue the conversation.", cls: "none" };
  }
}
...
export default function App() {
  ...
  async function handleNewSession() {
    ...
    try {
      ...
      console.warn("Could not start new check-in:", err);
      setError("Could not start a new check-in.");
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="app-shell">
        ...
        <div className="top-app-row">
          ...
          <div className="top-user-card">
            ...
            <div className="top-user-copy">
              ...
              <span className="top-user-sub">{isAdmin ? "Administrator" : "Current user"}</span>
            </div>
          </div>
        </div>
      ...
      <div className="app-body">
        <aside className={`sidebar ${sidebarOpen ? "sidebar-open" : ""}`}>
          ...
          <div className="sidebar-profile-compact">
            ...
            <div className="profile-copy">
              ...
              <div className="profile-sub">{isAdmin ? "Administrator" : "Current user"}</div>
            </div>
          </div>
            ...
            <>
              ...
              <section className="sidebar-section">
                ...
                <div className="actions-list">
                  <button className="action-item" onClick={() => void handleNewSession()} disabled={!userId || loading || bootstrapping}>
                    ...
                    <span><strong>Start new check-in</strong><small>Begin a fresh daily conversation</small></span>
                    <span className="action-arrow">›</span>
                  </button>

                  <button className="action-item" onClick={() => void handleRefresh()} disabled={!userId || loading || bootstrapping}>
                    <span className="action-icon blue">
                      <svg viewBox="0 0 24 24" fill="none">
                  ...
                  <button className="action-item danger" onClick={() => void handleClearChat()} disabled={!userId || loading}>
        ...
        <main className="main-panel">
          ...
          {activeView === "trends" && userId && !isAdmin ? (
            ...
            <ChatWindow
              ...
              onLoadMore={() => setVisibleCount((value) => value + 30)}
              hasMore={messages.length > visibleCount}
              onSend={handleSend}
