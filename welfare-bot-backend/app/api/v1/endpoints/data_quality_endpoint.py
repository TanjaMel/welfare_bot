...
Admin endpoint for data quality reports.
...
from app.services.data_quality import (
    check_user_data_quality,
    run_population_quality_check,
    repair_outliers,
    METRIC_COLUMNS,
)
...
@router.get(
    "/user/{user_id}",
    response_model=UserQualityResponse,
    summary="Data quality report for a single user",
)
def get_user_quality(
    user_id: int,
    assessment_date: Optional[date] = Query(default=None),
    lookback_days: int = Query(default=30, ge=7, le=90),
    db: Session = Depends(get_db),
):
    result = check_user_data_quality(
        user_id=user_id,
        db=db,
    ...
    return UserQualityResponse(
        ...
        metric_quality=[
            MetricQualityOut(
                metric=mq.metric,
                missing_rate=mq.missing_rate,
                outlier_count=mq.outlier_count,
                mean_value=mq.mean_value,
                quality_score=mq.quality_score,
            )
            for mq in result.metric_quality
        ],
        gaps=[
            GapOut(
...
@router.get(
    "/population",
    response_model=PopulationQualityResponse,
    summary="Population-level data quality report",
)
def get_population_quality(
    assessment_date: Optional[date] = Query(default=None),
    lookback_days: int = Query(default=30, ge=7, le=90),
    db: Session = Depends(get_db),
):
    result = run_population_quality_check(
        db=db,
        assessment_date=assessment_date,
        lookback_days=lookback_days,
    )
    return PopulationQualityResponse(
        assessment_date=result.assessment_date,
        total_users=result.total_users,
        users_with_data=result.users_with_data,
        users_needing_attention=result.users_needing_attention,
        avg_quality_score=result.avg_quality_score,
        avg_coverage_rate=result.avg_coverage_rate,
        most_missing_metric=result.most_missing_metric,
        longest_gap_days=result.longest_gap_days,
...
@router.post(
    ...
    summary="Detect and optionally repair outlier values for a user",
)
def repair_user_data(
    user_id: int,
    dry_run: bool = Query(default=True, description="If true, only report — do not modify data"),
